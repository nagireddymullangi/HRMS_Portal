...
public class BreakSession {
 ...
 public enum BreakType {
     ...
     BreakType(int minutes) {
         this.defaultMaxMinutes = minutes;
     }
 ...
 public enum Status {
     ACTIVE, COMPLETED, FORCE_STOPPED
 }

 // Helper: Calculate current duration if active
 public long getCurrentDurationMinutes() {
     ...
     if (startTime != null) {
         return Duration.between(startTime,
             endTime != null ? endTime : LocalDateTime.now()).toMinutes();
     }
     return 0;
 }
}